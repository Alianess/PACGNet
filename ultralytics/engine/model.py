...
class Model(nn.Module):
    ...
    def predict(
        ...
        **kwargs,
        ...
        return self.predictor.predict_cli(source=source) if is_cli else self.predictor(source=source, sourceIR=sourceIR, stream=stream)
    ...
    def profile(self, imgsz):
        ...
        if type(imgsz) is int:
            inputs = torch.randn((2, cn, imgsz, imgsz))
        else:
            inputs = torch.randn((2, cn, imgsz[0], imgsz[1]))
