...
# 没有购买yolov8项目需要注释以下
from ultralytics.nn.extra_modules import Detect_Efficient, Detect_DyHead_Prune, Detect_LSCD, Pose_LADH
...
class DetectionCompressor(BaseTrainer):
    ...
    def compress(self):
        ...
        if type(self.args.imgsz) is int:
            example_inputs = torch.randn((1, cn, self.args.imgsz, self.args.imgsz)).to(self.device)
        elif type(self.args.imgsz) is list:
            example_inputs = torch.randn((1, cn, self.args.imgsz[0], self.args.imgsz[1])).to(self.device)
        else:
