...
# 没有购买yolov8项目需要注释以下
from ultralytics.nn.extra_modules import Detect_Efficient, Detect_DyHead_Prune, Pose_LADH
...
def get_pruner(opt, model, example_inputs):
    ...
    # for yolov8.yaml        Pose_LADH
    for k, m in model.named_modules():
        # print(f"m is {m}")
        # if isinstance(m, Pose):
        if isinstance(m, Pose_LADH):
            print(f"m is {m}")
            ...
            ignored_layers.append(m.cv4[0][3])
            ignored_layers.append(m.cv4[1][3])
            ignored_layers.append(m.cv4[2][3])
