...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Returns:
            (torch.Tensor): The last output of the model.
        """
        y, dt, gflops, para, embeddings = [], [], [], [], []  # outputs
        ...
        if IR_support == True:
            ...
            for m in self.model:
                ...
                #     print(f"Layer {m.__class__.__name__}input: Tensor shape -> {x[0].shape}     {x[1].shape}")
                # else:
                #     print(f"Layer {m.__class__.__name__}input: Tensor shape -> {x.shape}")
                ...
                if profile:
                    ...
                    self._profile_one_layer(m, x, dt, gflops, para)
                ...
                if m.f == -4:
                    ...
                    if (len(list(m.named_parameters())) > 0):
                        ...
                        x = []  # 中间层
                elif in_backbone == True and m.__class__.__name__ != "ADD":
                    if isR:
                        x = m(rgb)
                        rgb = x
                    else:
    ...
    def _profile_one_layer(self, m, x, dt, gflops, para):
        """
        ...
        gflops.append(o)
        para.append(get_num_params(m))
        if c:
            LOGGER.info(f"{sum(dt):10.2f} {sum(gflops):10.2f} {sum(para):10.0f}  Total")
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    # "IR_support"
    tx = [3, 256, 256, 512, 512, max_channels,max_channels]
    ty = 0
    # "IR_support END"
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, C2PSA, DWConv, Focus, BottleneckCSP, C1, C2, C2f, C3k2, A2C2f, ELAN1, AConv, SPPELAN, C2fAttn, C3, C3TR,
            ...
            if m in {
                     BottleneckCSP, C1, C2, C2f, C3k2, A2C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2PSA, C2fCIB, C2f_Faster, C2f_ODConv, C2f_Faster_EMA, C2f_DBB,
