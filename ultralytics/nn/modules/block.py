...
class HierarchicalSpatialGate(nn.Module):
    """
    分层空间门控：(与之前版本相同)
    - 输入: 上一层级 (P(i-1)) 的 RGB, IR 联合特征 (B, 2*C_prev, H_prev, W_prev)
    - 输出: 当前层级 (Pi) 分辨率的 Gate 掩码 (B, 1, H_prev/2, W_prev/2)
    """

    def __init__(self, channels_prev, gate_channels=16, kernel_size=3):
        super(HierarchicalSpatialGate, self).__init__()
        # Stride=2 的卷积核，用于降维和提取特征
        pad = kernel_size // 2
    ...
    def forward(self, x_rgb_prev, x_ir_prev):
        ...
        return self.gate(combined)  # 输出分辨率为 H_prev/2, W_prev/2
...
# class ADD(nn.Module):
#     """
#     使用Mamba思想，对两个输入张量进行动态、选择性的加权融合。

#     这个类不再执行简单的 `x[0] + x[1]`。相反，它利用Mamba模型根据
#     输入内容生成一个门控（gate），然后使用该门控来动态地混合两个张量。
#     这体现了Mamba的核心思想：input-dependent selection（依赖于输入的选择）。
#     """

#     def __init__(self,
#                  d_model,  # 输入张量的特征维度 (例如 128, 256, 512)
#                  d_state=16,  # Mamba的SSM状态维度
#                  d_conv=4,  # Mamba的1D卷积核大小
#                  expand=2):  # Mamba块的扩展因子
#         """
#         初始化函数。

#         Args:
#             d_model (int): 输入张量的特征维度（即通道数或嵌入维度）。
#                            原始代码中的'arg'参数现在有了明确的含义。
#             d_state (int): SSM (A, B, C, D) 矩阵的状态维度。
#             d_conv (int): 内部1D卷积的核大小。
#             expand (int): 块内特征维度的扩展因子。
#         """
#         super(ADD, self).__init__()
#         self.d_model = d_model

#         # 1. Mamba核心模块：用于处理两个输入的组合信息，并学习其上下文依赖关系。
#         # 我们将两个输入相加作为Mamba的输入，以创建一个联合表示。
#         self.mamba = Mamba(
#             d_model=d_model,
#             d_state=d_state,
#             d_conv=d_conv,
...
#         # 2. 门控生成模块：将Mamba的输出转换为一个0到1之间的门控值。
#         # Sigmoid激活函数非常适合这个任务。
...
#         Args:
#             x (tuple or list): 包含两个形状相同的张量，例如 (x1, x2)。
#                                张量的期望形状为 (batch_size, seq_len, d_model)。

#         Returns:
#             torch.Tensor: 两个输入张量经过Mamba选择性融合后的结果。
#         """
#         x1, x2 = x[0], x[1]
...
#         # 步骤 1: 将两个输入结合，创建Mamba的输入上下文。
#         # 简单的相加是一种有效的结合方式，也可以使用拼接等其他方法。
#         combined_context = x1 + x2

#         # 步骤 2: 将上下文送入Mamba模块进行处理。
#         # Mamba会捕捉序列中的长距离依赖和局部上下文。
#         mamba_out = self.mamba(combined_context)

#         # 步骤 3: 从Mamba的输出生成门控张量g。
#         # g的每个元素值都在 (0, 1) 区间内。
#         gate = self.gating_head(mamba_out)

#         # 步骤 4: 应用门控，进行选择性融合。
#         # output = gate * x1 + (1 - gate) * x2
#         # 当gate中的某个元素接近1时，该位置的输出更依赖于x1。
#         # 当gate中的某个元素接近0时，该位置的输出更依赖于x2。
#         # 这就是“选择性”的体现。
