...
class yolo_heatmap:
    ...
    def process(self, img_path, imgIR_path, save_path, style):
        # img process
        try:
            imgRGB = cv2.imdecode(np.fromfile(img_path, np.uint8), cv2.IMREAD_COLOR)
        except:
            print(f"Warning... {img_path} read failure.")
            return
        imgRGB, _, (top, bottom, left, right) = letterbox(imgRGB, new_shape=(self.img_size, self.img_size),
                                                       auto=True)  # 如果需要完全固定成宽高一样就把auto设置为False
        imgIR = cv2.imread(imgIR_path)

        img = np.dstack((imgRGB, imgIR))
        img = [np.stack([x for x in img])]
        img = np.array(img)
        img = img[..., ::-1].transpose((0, 3, 1, 2))
        img = np.ascontiguousarray(img[0])  # contiguous
        img = np.float32(img) / 255.0
        imgIR = np.float32(imgIR) / 255.0
        imgRGB = np.float32(imgRGB) / 255.0
        ...
        try:
            ...
            import traceback
            traceback.print_exc()  # 打印详细的错误堆栈信息，看看AttributeError具体发生在哪一行

        grayscale_cam = grayscale_cam[0, :]
        cam_image = show_cam_on_image(IR_RGB_img, grayscale_cam, use_rgb=True, image_weight=0.45)
    ...
    def __call__(self, img_path, imgIR_path, save_path, style):
        # remove dir if exist
        if os.path.exists(save_path):
            shutil.rmtree(save_path)
        # make dir if not exist
        ...
        if os.path.isdir(img_path):
            ...
            self.process(img_path, imgIR_path, f'{save_path}/result.png', style)

# runs/train/v8n-5.22-RIFusion-CNN-ADDCNN/weights/best.pt
# runs/train/v8n-5.21-ADD/weights/best.pt
def get_params():
    params = {
        'weight': 'runs/train/VEDAI-RIFusion-ADD-10244/weights/best.pt', # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    # index = "01039"
    index = "00000288"
    # model(f'dataset/OBBCrop/images/test/{index}.jpg', f'dataset/OBBCrop/imageIR/test/{index}.jpg', 'result', style="IR")
    model(f'dataset/VEDAI/images/train/{index}.png', f'dataset/VEDAI/imageIR/train/{index}.png', 'result', style="IR")
